...
std::string drawer::draw_helper(int i_factor, int i_step, int j_factor, int j_step) const {
    ...
    std::ostringstream oss; // output string stream to accumulate the output
...
scale::scale(artist *artist_, int x_ratio, int y_ratio)
 : drawer(artist_) {
     // calculate the stretching factor and step for the i-axis (vertical)
    i_length_factor = max(1, y_ratio);
    i_step = max(1, -y_ratio);

     // calculate the stretching factor and step for the j-axis (horizontal)
    j_length_factor = max(1, x_ratio);
    j_step = max(1, -x_ratio);
