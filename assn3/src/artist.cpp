...
char classic::mapper(int x, int y) const {
    return TOKENS[min(get_pixel(x, y) / (int) (PIXEL_MAX / TOKEN_SIZE), TOKEN_SIZE - 1)]; // map pixel value to a character
...
char iclassic::mapper(int x, int y) const {
    return TOKENS[(TOKEN_SIZE - 1) - min(get_pixel(x, y) / (int) (PIXEL_MAX / TOKEN_SIZE), TOKEN_SIZE - 1)]; // map pixel value to a character (reversed order of classic::mapper())
}
...
char digit::mapper(int x, int y) const {
    return DIGITS[min(get_pixel(x, y) / (int) (PIXEL_MAX / DIGIT_SIZE), DIGIT_SIZE - 1)]; // map pixel value to a character (0-9 decimal digits)
}
